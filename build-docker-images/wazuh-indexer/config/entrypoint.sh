...
# Stores snapshot repository for Amazon S3
if [[ -n "$S3_ACCESS_KEY" && -n "$S3_SECRET_KEY" ]]; then
  ...
  if ! (run_as_other_user_if_needed ./bin/opensearch-keystore has-passwd --silent) ; then
    # keystore is unencrypted
    if ! (run_as_other_user_if_needed ./bin/opensearch-keystore list | grep -q '^s3.client.default.access_key$'); then
      (run_as_other_user_if_needed echo "$S3_ACCESS_KEY" | ./bin/opensearch-keystore add -x 's3.client.default.access_key')
    fi
    if ! (run_as_other_user_if_needed ./bin/opensearch-keystore list | grep -q '^s3.client.default.secret_key$'); then
      (run_as_other_user_if_needed echo "$S3_SECRET_KEY" | ./bin/opensearch-keystore add -x 's3.client.default.secret_key')
    fi
  else
    # keystore requires password
    if ! (run_as_other_user_if_needed echo "$KEYSTORE_PASSWORD" \
        | ./bin/opensearch-keystore list | grep -q '^s3.client.default.access_key$') ; then
      COMMANDS="$(printf "%s\n%s" "$KEYSTORE_PASSWORD" "$S3_ACCESS_KEY")"
      (run_as_other_user_if_needed echo "$COMMANDS" | ./bin/opensearch-keystore add -x 's3.client.default.access_key')
    fi
    if ! (run_as_other_user_if_needed echo "$KEYSTORE_PASSWORD" \
        | ./bin/opensearch-keystore list | grep -q '^s3.client.default.secret_key$') ; then
      COMMANDS="$(printf "%s\n%s" "$KEYSTORE_PASSWORD" "$S3_SECRET_KEY")"
      (run_as_other_user_if_needed echo "$COMMANDS" | ./bin/opensearch-keystore add -x 's3.client.default.secret_key')
    fi
  fi
fi
